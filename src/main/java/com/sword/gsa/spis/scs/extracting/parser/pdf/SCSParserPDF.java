...
import com.sword.gsa.spis.scs.extracting.parser.SCSAbstractParser;
...
public class SCSParserPDF extends SCSAbstractParser implements Initializable {

    private static volatile boolean HAS_WARNED = false;
    private static final Object[] LOCK = new Object[0];

    private static final Logger logger = LoggerFactory.getLogger(SCSParserPDF.class);

    public static final String PASSWORD = "org.apache.extracting.parser.pdf.password";
    ...
    private SCSParserPDFConfig defaultConfig = new SCSParserPDFConfig();
    ...
    public void parse(InputStream stream, ContentHandler handler, Metadata metadata, ParseContext context)
        ...
        logger.info(String.format("PDF recognized, the PDF parser is used"));
        SCSParserPDFConfig localConfig = context.get(SCSParserPDFConfig.class, defaultConfig);
        ...
        try {
            ...
            if (handler != null) {
                ...
                } else if (localConfig.getOcrStrategy().equals(SCSParserPDFConfig.OCR_STRATEGY.OCR_ONLY)) {
                    metadata.add("X-Parsed-By", TesseractOCRParser.class.toString());
                    SCSOCR2XHTML.process(pdfDocument, handler, context, metadata, localConfig);
                } else {
                    if (localConfig.getOcrStrategy().equals(SCSParserPDFConfig.OCR_STRATEGY.OCR_AND_TEXT_EXTRACTION)) {
                        metadata.add("X-Parsed-By", TesseractOCRParser.class.toString());
                    }
                    SCSPDF2XHTML.process(pdfDocument, handler, context, metadata, localConfig);
                }
            }
    ...
    private void extractMetadata(PDDocument document, Metadata metadata, ParseContext context) throws TikaException {
        ...
        List<String> handledMetadata =
                Arrays.asList("Author", "Creator", "CreationDate", "ModDate", "Keywords", "Producer", "Subject", "Title", "Trapped");
        ...
        // There are also three potential pdf specific version keys: pdf:PDFVersion, pdfa:PDFVersion,
    ...
    /**
     * This tries to read a list from a particular property in XMPSchemaDublinCore. If it can't find the
     * information, it falls back to the pdfboxBaseline. The pdfboxBaseline should be the value that
     * pdfbox returns from its PDDocumentInformation object (e.g. getAuthor()) This method is designed
     * include the pdfboxBaseline, and it should not duplicate the pdfboxBaseline.
     * <p/>
    ...
    /**
     * As of this writing, XMPSchema can contain bags or sequence lists for some attributes...despite
     * standards documentation. JempBox expects one or the other for specific attributes. Until more
     * flexibility is added to JempBox, Tika will have to handle both.
     *
     * @param schema
     * @param name
    ...
    private String decode(String value) {
        if (SCSEncodedPDFStringDecoder.shouldDecode(value)) {
            SCSEncodedPDFStringDecoder d = new SCSEncodedPDFStringDecoder();
    ...
    /**
     * Used when processing custom metadata entries, as PDFBox won't do the conversion for us in the way
     * it does for the standard ones
     */
    ...
    private boolean shouldHandleXFAOnly(PDDocument pdDocument, SCSParserPDFConfig config) {
        if (config.getIfXFAExtractOnlyXFA() && pdDocument.getDocumentCatalog() != null && pdDocument.getDocumentCatalog().getAcroForm() != null
    ...
    private void handleXFAOnly(PDDocument pdDocument, ContentHandler handler, Metadata metadata, ParseContext context)
            throws SAXException, IOException, TikaException {
        SCSExtractorXFA ex = new SCSExtractorXFA();
    ...
    public SCSParserPDFConfig getPDFParserConfig() {
        return defaultConfig;
    }

    public void setPDFParserConfig(SCSParserPDFConfig config) {
     ...
     * For many PDFs this is necessary as they do not include explicit whitespace characters.
     *
     * @deprecated use {@link #setPDFParserConfig(SCSParserPDFConfig)}
     */
    ...
    /**
     * If true, the parser should try to remove duplicated text over the same region. This is needed for
     * some PDFs that achieve bolding by re-writing the same text in the same area. Note that this can
     * slow down extracting substantially (PDFBOX-956) and sometimes remove characters that were not in
    ...
    /**
     * If true, sort text tokens by their x/y position before extracting text. This may be necessary for
     * some PDFs (if the text tokens are not rendered "in order"), while for other PDFs it can produce
     * the wrong result (for example if there are 2 columns, the text will be interleaved). Default is
     * false.
     *
     * @deprecated use {@link #setPDFParserConfig(SCSParserPDFConfig)}
     */
    ...
    /**
     * This is a no-op. There is no need to initialize multiple fields. The regular field loading should
     * happen without this.
     *
        ...
        synchronized (LOCK) {
            ...
            if (sb.length() > 0) {
                InitializableProblemHandler localInitializableProblemHandler =
                        (initializableProblemHandler == null) ? handler : initializableProblemHandler;
                localInitializableProblemHandler.handleInitializableProblem("com.sword.gsa.spis.scs.extracting.parser.pdf.SCSParserPDF",
                        sb.toString());
            }
            HAS_WARNED = true;
        }
